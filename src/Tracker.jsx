...
export default function App() {
  ...
  const dataBar = (
    <div style={{ background: C.card, borderBottom: "1px solid " + C.line, padding: "12px 16px" }}>
  ...
  const banner = storageOk === false ? (
    <div style={{ background: "#3A2420", borderBottom: "1px solid " + C.line, padding: "10px 16px", fontFamily: "'Helvetica Neue',sans-serif", fontSize: 12, color: "#E8A87C" }}>
...
function Splash({ onDone }) {
  ...
  return (
    <div onClick={onDone} style={{ position: "fixed", inset: 0, background: C.paper, zIndex: 9999 }}>
      <img src="/splash.jpg" alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
      <div style={{ position: "absolute", inset: 0, background: "linear-gradient(180deg, rgba(0,0,0,0) 55%, rgba(0,0,0,.85) 100%)" }} />
      <div style={{ position: "absolute", left: 0, right: 0, bottom: "max(28px, env(safe-area-inset-bottom))", textAlign: "center" }}>
        <div style={{ fontFamily: "'Helvetica Neue',sans-serif", fontWeight: 800, fontSize: 26, letterSpacing: ".03em", color: "#fff", textTransform: "uppercase" }}>Let's fucking go</div>
      </div>
    </div>
  );
}
...
function Frame({ children }) {
  return (
    <div style={{ minHeight: "100vh", background: C.paper, display: "flex", justifyContent: "center", padding: "16px 0", fontFamily: "'Georgia', serif" }}>
      <div style={{ width: "100%", maxWidth: 430, background: C.paper, borderRadius: 28, overflow: "hidden", boxShadow: "0 12px 40px rgba(0,0,0,.35)", border: "1px solid " + C.line }}>
        {children}
      </div>
    </div>
  );
